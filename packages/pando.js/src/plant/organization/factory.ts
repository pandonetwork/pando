...
export default class OrganizationFactory {
  ...
  public async deploy(name: string): Promise<any> {
    ...
    const apm = APM(new Web3(this.plant.pando.options.ethereum.provider), {
  ...
  public async add(name: string, address: string): Promise<Organization> {
    ...
    const kernel = await this.plant.pando.contracts.Kernel.at(address)
    const acl = await this.plant.pando.contracts.ACL.at(await kernel.acl())
  ...
  public async delete({ name, address }: { name?: string; address?: string } = {}): Promise<void> {
    ...
    await fs.remove(npath.join(this.plant.paths.organizations, (address as string) + '.db'))
  }
