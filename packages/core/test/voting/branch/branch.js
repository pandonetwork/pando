...
contract('Branch App', accounts => {
    ...
    //         const requestId = submittedRequestId(await master.submitRequest('awesomeIpfsHash', 'First commit', parents, { from: authorized }))
    //         const request = await master.getRequest(requestId)
    //
    //         assert.equal(request.origin, authorized)
    //         assert.equal(request.block, 0)
    //         assert.equal(request.tree, 'awesomeIpfsHash')
    //         assert.equal(request.message, 'First commit')
    //         assert.equal(request.parents, parents)
    //         assert.equal(request.value, 0)
    //         assert.equal(request.state, REQUEST_STATE.PENDING)
    //         assert.equal(request.status, REQUEST_STATUS.OPEN)
    //     })
    //
    //     it("should fail to submit request if address is not granted 'SUBMIT_REQUEST_ROLE'", async () => {
    //         return assertRevert(async () => {
    //             await master.submitRequest('awesomeIpfsHash', 'First commit', parents, { from: unauthorized })
    //         })
    //     })
    // })
    //
    ...
    //         requestId = submittedRequestId(await master.submitRequest('awesomeIpfsHash', 'First commit', parents, { from: origin }))
    //     })
    //
    ...
    //         const request = await master.getRequest(requestId)
    //
    //         assert.equal(request.state, REQUEST_STATE.VALUATED)
    //         assert.equal(request.value, 0)
    //     })
    //
    ...
    //         return assertRevert(async () => {
    //             await master.valuateRequest(requestId, 50, { from: unauthorized })
    //         })
    //     })
    //
    ...
    //         return assertRevert(async () => {
    //             await master.valuateRequest(requestId, 50, { from: origin })
    //         })
    //     })
    //
    ...
    context('#handleRequest', () => {
        ...
        context('Merge', () => {
            ...
            it("should succeed to merge request if request is open and request is valuated and address is granted 'HANDLE_REQUEST_ROLE'", async () => {
                await master.valuateRequest(requestId, 50, { from: authorized })
                await master.handleRequest(requestId, REQUEST_ACTION.MERGE, { from: authorized })

                const request = await master.getRequest(requestId)

                assert.equal(request.state, REQUEST_STATE.HANDLED)
                assert.equal(request.status, REQUEST_STATUS.MERGED)
            })

            it("should fail to merge request if address is not granted 'HANDLE_REQUEST_ROLE' even if request is open and valuated", async () => {
                await master.valuateRequest(requestId, 50, { from: authorized })
                return assertRevert(async () => {
                    await master.handleRequest(requestId, REQUEST_ACTION.MERGE, { from: unauthorized })
                })
            })

            it("should fail to merge request if request is not open even if address is granted 'HANDLE_REQUEST_ROLE'", async () => {
                await master.valuateRequest(requestId, 50, { from: authorized })
                await master.handleRequest(requestId, REQUEST_ACTION.MERGE, { from: authorized })
                // request is already merged and thus not open anymore
                return assertRevert(async () => {
                    await master.handleRequest(requestId, REQUEST_ACTION.MERGE, { from: authorized })
                })
            })

            it("should fail to merge request if request is not valuated even if request is open and address is granted 'HANDLE_REQUEST_ROLE'", async () => {
                return assertRevert(async () => {
                    await master.handleRequest(requestId, REQUEST_ACTION.MERGE, { from: authorized })
                })
            })
        ...
        context('Reject', () => {
            it("should succeed to reject request if request is open and address is granted 'HANDLE_REQUEST_ROLE'", async () => {
                await master.handleRequest(requestId, REQUEST_ACTION.REJECT, { from: authorized })

                const request = await master.getRequest(requestId)

                assert.equal(request.state, REQUEST_STATE.HANDLED)
                assert.equal(request.status, REQUEST_STATUS.REJECTED)
            })

            it("should fail to reject request if address is not granted 'HANDLE_REQUEST_ROLE' even if request is open", async () => {
                return assertRevert(async () => {
                    await master.handleRequest(requestId, REQUEST_ACTION.REJECT, { from: unauthorized })
                })
            })

            it("should fail to reject request if request is not open even if address is granted 'HANDLE_REQUEST_ROLE'", async () => {
                await master.handleRequest(requestId, REQUEST_ACTION.REJECT, { from: authorized })
        ...
        context('Cancel', () => {
            it("should succeed to cancel request if request is open and address is granted 'HANDLE_REQUEST_ROLE'", async () => {
                await master.handleRequest(requestId, REQUEST_ACTION.CANCEL, { from: authorized })

                const request = await master.getRequest(requestId)

                assert.equal(request.state, REQUEST_STATE.HANDLED)
                assert.equal(request.status, REQUEST_STATUS.CANCELLED)
            })

            it("should fail to cancel request if address is not granted 'HANDLE_REQUEST_ROLE' even if request is open", async () => {
                return assertRevert(async () => {
                    await master.handleRequest(requestId, REQUEST_ACTION.CANCEL, { from: unauthorized })
                })
            })

            it("should fail to cancel request if request is not open even if address is granted 'HANDLE_REQUEST_ROLE'", async () => {
                await master.handleRequest(requestId, REQUEST_ACTION.CANCEL, { from: authorized })
                // request is already rejected and thus not open anymore
                return assertRevert(async () => {
                    await master.handleRequest(requestId, REQUEST_ACTION.CANCEL, { from: authorized })
