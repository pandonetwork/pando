...
export const prompt = {
  configure: async (): Promise<any> => {
    ...
    configuration.ethereum.connection = (await inquirer.prompt(questions.ethereum.frame)).result === 'Frame' ? 'frame' : 'direct'
    ...
    const provider = configuration.ethereum.connection === 'frame' ? ETHProvider(FRAME_ENDPOINT) : ETHProvider(_url(configuration))
    const message  = configuration.ethereum.connection === 'frame' ? chalk.bold('Connecting to Frame') : chalk.bold('Connecting to Ethereum gateway')
